...
@app.route("/api/intel-hub")
def api_intel_hub():
    """Consolidated Intel Hub endpoint with data status flags for every NIIN."""
    db = get_db()
    page = int(request.args.get("page", 1))
    per_page = int(request.args.get("per_page", 50))
    search = request.args.get("search", "").strip()
    action_filter = request.args.get("action", "").strip()
    _min_d = request.args.get("min_demand", "").strip()
    min_demand = int(_min_d) if _min_d else 0
    sort_col = request.args.get("sort", "opportunity_score")
    sort_dir = request.args.get("order", "desc").upper()
    if sort_dir not in ("ASC", "DESC"):
        sort_dir = "DESC"
    missing_only = request.args.get("missing_only", "").strip()

    # Build query from command_center as the primary source
    # LEFT JOIN other tables to get data status flags
    where = ["1=1"]
    params = []

    if search:
        where.append("(cc.niin LIKE ? OR cc.item_name LIKE ?)")
        params.extend([f"%{search}%", f"%{search}%"])
    if action_filter:
        where.append("cc.action = ?")
        params.append(action_filter)
    if min_demand > 0:
        where.append("cc.total_demand >= ?")
        params.append(min_demand)

    # missing_only filter applied post-join
    where_sql = " AND ".join(where)

    # Safe sort columns
    safe_sorts = {
        "opportunity_score": "cc.opportunity_score",
        "niin": "cc.niin",
        "item_name": "cc.item_name",
        "action": "cc.action",
        "total_demand": "cc.total_demand",
        "cots_readiness": "cc.cots_readiness",
        "govt_unit_price": "cc.govt_unit_price",
    }
    sort_expr = safe_sorts.get(sort_col, "cc.opportunity_score")

    # Core subquery with all JOINs
    base_sql = f"""
        SELECT cc.niin, cc.item_name, cc.action, cc.opportunity_score, cc.cots_readiness,
               cc.demand_score, cc.char_score, cc.supply_score, cc.price_score, cc.competition_score,
               cc.total_demand, cc.confidence,
               cc.govt_unit_price, cc.median_unit_price,
               CASE WHEN f.niin IS NOT NULL THEN 1 ELSE 0 END as has_forecast,
               CASE WHEN pi.niin IS NOT NULL THEN 1 ELSE 0 END as has_publog,
               CASE WHEN ps.niin IS NOT NULL THEN 1 ELSE 0 END as has_pricing,
               CASE WHEN fp.niin IS NOT NULL THEN 1 ELSE 0 END as has_fingerprint,
               CASE WHEN cc.cots_readiness > 0 THEN 1 ELSE 0 END as has_cots,
               CASE WHEN pa.niin IS NOT NULL THEN 1 ELSE 0 END as has_profit,
               CASE WHEN cj.niin IS NOT NULL THEN 1 ELSE 0 END as has_cad,
               CASE WHEN cr.niin IS NOT NULL THEN 1 ELSE 0 END as has_search
        FROM command_center cc
        LEFT JOIN forecast f ON cc.niin = f.niin
        LEFT JOIN publog_identification pi ON cc.niin = pi.niin
        LEFT JOIN pricing_signals ps ON cc.niin = ps.niin
        LEFT JOIN fingerprint_by_niin fp ON cc.niin = fp.niin
        LEFT JOIN profit_assessments pa ON cc.niin = pa.niin
        LEFT JOIN (SELECT DISTINCT niin FROM cad_jobs) cj ON cc.niin = cj.niin
        LEFT JOIN (SELECT DISTINCT niin FROM crawler_results) cr ON cc.niin = cr.niin
        WHERE {where_sql}
    """

    # missing_only: filter to only show NIINs missing at least one data source
    if missing_only == "1":
        count_sql = f"SELECT COUNT(*) c FROM ({base_sql}) sub WHERE has_forecast=0 OR has_publog=0 OR has_pricing=0 OR has_fingerprint=0 OR has_cots=0 OR has_profit=0 OR has_cad=0 OR has_search=0"
        total = db.execute(count_sql, params).fetchone()["c"]
        rows = db.execute(f"""
            SELECT * FROM ({base_sql}) sub
            WHERE has_forecast=0 OR has_publog=0 OR has_pricing=0 OR has_fingerprint=0 OR has_cots=0 OR has_profit=0 OR has_cad=0 OR has_search=0
            ORDER BY {sort_expr.replace('cc.', '')} {sort_dir}
            LIMIT ? OFFSET ?
        """, params + [per_page, (page - 1) * per_page]).fetchall()
    else:
        total = db.execute(f"SELECT COUNT(*) c FROM command_center cc WHERE {where_sql}", params).fetchone()["c"]
        rows = db.execute(f"""
            {base_sql}
...
# ──────────────────────────────────────────────
# SECTION I-B-4: Google Search API (replaces HTTP crawler)
# ──────────────────────────────────────────────

def _google_search_for_niin(niin, item_name, part_numbers, characteristics=None):
    """Use Gemini with Google Search grounding to find tech data for a NIIN.
    Returns list of dicts with url, title, snippet, relevance."""
    from google import genai
    ...
    pn_text = ", ".join(part_numbers[:3]) if part_numbers else ""
    char_text = ""
    if characteristics:
        char_lines = []
        for c in characteristics[:5]:
            stmt = c.get("requirements_statement", "")
            reply = c.get("clear_text_reply", "")
            if stmt and reply:
                char_lines.append(f"{stmt}: {reply}")
    ...
    prompt = (
        f"Find technical specifications, datasheets, suppliers, military technical manuals, "
        f"and commercial sources for this military part:\n"
        f"NIIN: {niin}\nItem: {item_name}\n"
    )
    if pn_text:
        prompt += f"Part Numbers: {pn_text}\n"
    if char_text:
        prompt += f"Key specs: {char_text}\n"
    prompt += (
        f"\nProvide:\n1. Links to technical manuals or datasheets\n"
        f"2. Commercial suppliers that sell this or equivalent parts\n"
        f"3. Any MIL-SPEC or standards references\n"
        f"4. OEM manufacturer information\n"
        f"\nFor each source found, provide the URL, a brief description, and how relevant it is."
    )
            ...
            if hasattr(gm, 'grounding_chunks') and gm.grounding_chunks:
                for chunk in gm.grounding_chunks:
                    if hasattr(chunk, 'web') and chunk.web:
                        ...
                        # Score relevance based on content
                        relevance = 30.0
                        url_lower = url.lower()
                        title_lower = title.lower()
                        if niin in title_lower or niin in url_lower:
                            relevance += 40
                        for pn in part_numbers[:3]:
                            if pn.lower() in title_lower:
                                relevance += 20
                        for kw in ("datasheet", "specification", "mil-spec", "technical manual",
                                   "drawing", "pdf", "catalog"):
                            if kw in title_lower or kw in url_lower:
                                relevance += 5
                        results.append({
                            "url": url, "title": title,
                            "snippet": "", "relevance": min(100, relevance)
                        })

    # Also parse the AI summary for additional context
    if summary_text:
        results.append({
            "url": "", "title": "AI Research Summary",
            "snippet": summary_text[:2000], "relevance": 50
        })

    return results


def run_google_search_worker(log_id, niin):
    """Search for technical data using Google Search API via Gemini."""
    try:
        log.info("Starting Google Search for NIIN %s", niin)
        db = _worker_db()

        # Get NIIN info
        ident = db.execute("SELECT item_name FROM publog_identification WHERE niin=?", (niin,)).fetchone()
        item_name = ident["item_name"] if ident else ""
        refs = db.execute("SELECT DISTINCT part_number FROM publog_references WHERE niin=?", (niin,)).fetchall()
        pns = [r["part_number"] for r in refs if r["part_number"]]
        chars = db.execute("""
            SELECT requirements_statement, clear_text_reply
            FROM publog_characteristics WHERE niin=? LIMIT 10
        """, (niin,)).fetchall()
        chars_list = [dict(c) for c in chars]

        # Run Google Search
        results = _google_search_for_niin(niin, item_name, pns, chars_list)

        # Store results in crawler_results table
        stored = 0
        for r in results:
            if r.get("url") or r.get("snippet"):
                db.execute("""
                    INSERT INTO crawler_results
                    (niin, site_name, source_url, page_url, page_title,
                     snippet, found_pns, found_specs, relevance_score)
                    VALUES (?,?,?,?,?,?,?,?,?)
                """, (niin, "Google Search", r.get("url", ""), r.get("url", ""),
                      r.get("title", ""), r.get("snippet", "")[:2000],
                      ",".join(pns[:5]), "", r.get("relevance", 50)))
                stored += 1

        db.commit()
        db.close()
        _update_log(log_id, "success", stored,
                    f"Google Search for NIIN {niin}: {stored} results found")
        log.info("Google Search completed for NIIN %s: %d results", niin, stored)
    except Exception as e:
        log.exception("Google Search failed for NIIN %s", niin)
        _update_log(log_id, "error", log_text=str(e)[:500])


def run_google_search_batch_worker(log_id, niins):
    """Batch Google Search for multiple NIINs."""
    try:
        db = _worker_db()
        total = 0
        errors = []
        for niin in niins:
            try:
                ...
                refs = db.execute("SELECT DISTINCT part_number FROM publog_references WHERE niin=?", (niin,)).fetchall()
                pns = [r["part_number"] for r in refs if r["part_number"]]
                results = _google_search_for_niin(niin, item_name, pns)
                for r in results:
                    if r.get("url") or r.get("snippet"):
                        db.execute("""
                            INSERT INTO crawler_results
                ...
                errors.append(f"{niin}: {str(ex)[:80]}")
        db.close()
        err_text = f" Errors: {len(errors)}" if errors else ""
        _update_log(log_id, "success", total,
                    f"Searched {len(niins)} NIINs: {total} results.{err_text}")
    except Exception as e:
        log.exception("Batch Google Search failed")
        _update_log(log_id, "error", log_text=str(e)[:500])


@app.route("/api/run/google-search", methods=["POST"])
def api_run_google_search():
    """Google Search for a single NIIN."""
    data = request.json if request.is_json else {}
    niin = data.get("niin")
    if not niin:
        return jsonify({"error": "niin required"}), 400
    log_id = _start_pipeline("google_search", run_google_search_worker, niin)
    return jsonify({"job_id": log_id, "status": "running"})


@app.route("/api/run/google-search-batch", methods=["POST"])
def api_run_google_search_batch():
    """Batch Google Search for multiple NIINs. Provide either niins list or max_niins count."""
    data = request.json if request.is_json else {}
    niins = data.get("niins", [])
    max_niins = int(data.get("max_niins", 0))
    if not niins and max_niins > 0:
        # Auto-select top NIINs by opportunity score that don't have search results yet
        db = get_db()
        rows = db.execute("""
            SELECT cc.niin FROM command_center cc
            LEFT JOIN (SELECT DISTINCT niin FROM crawler_results) cr ON cc.niin = cr.niin
            WHERE cr.niin IS NULL AND cc.action != 'PASS'
            ORDER BY cc.opportunity_score DESC
            LIMIT ?
        """, (max_niins,)).fetchall()
        db.close()
        niins = [r["niin"] for r in rows]
    if not niins:
        return jsonify({"error": "No NIINs to search. Provide niins list or max_niins count."}), 400
    log_id = _start_pipeline("google_search_batch", run_google_search_batch_worker, niins)
    return jsonify({"job_id": log_id, "status": "running", "count": len(niins)})
